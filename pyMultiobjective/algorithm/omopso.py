...
def put_value_in_function(input_list,function):
    x,y,z = symbols("x y z")
    return function.copy().subs(x,input_list[0]).subs(y,input_list[1]).subs(z,input_list[2])

############################################################################

# Function: Initialize Variables
def initial_position(swarm_size, min_values  , max_values   , list_of_functions    ):
    position = np.zeros((swarm_size, len(min_values) + len(list_of_functions)))
    for i in range(0, swarm_size):
        for j in range(0, len(min_values)):
             position[i,j] = random.uniform(min_values[j], max_values[j])      
        for k in range (1, len(list_of_functions) + 1):
            # position[i,-k] = list_of_functions[-k](list(position[i,0:position.shape[1]-len(list_of_functions)])) # list of functions is unknown at the beginning in TCP calibration case
            position[i,-k] = put_value_in_function(list(position[i,0:position.shape[1]-len(list_of_functions)]),list_of_functions[-k])
...
# Function: Velocity
def velocity_vector(position, leaders, min_values  , max_values   , list_of_functions    ):
    ...
    for i in range(0, vel_.shape[0]):
        ...
        for k in range (1, len(list_of_functions) + 1):
            ...
            vel_[i,-k] = put_value_in_function(list(vel_[i,0:vel_.shape[1]-len(list_of_functions)]),list_of_functions[-k])
    return vel_
...
# Function: Mutation
def mutation(position, mutation_rate, eta , min_values, max_values   , list_of_functions):
    d_mutation = 0            
    for i in range (0, position.shape[0]):
        ...
        for k in range (1, len(list_of_functions) + 1):
            ...
            position[i,-k] = put_value_in_function(position[i,0:position.shape[1]-len(list_of_functions)],list_of_functions[-k])
...
# OMPSO Function
def optimized_multiobjective_particle_swarm_optimization(swarm_size, min_values  , max_values   , iterations , list_of_functions    , mutation_rate , eta , verbose = True):    
    count    = 0
